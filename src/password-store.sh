...
cmd_show() {
	...
	if [[ -f $passfile ]]; then
		...
		tree -N -C -l --noreport -a -I ".git/" -I ".gitignore" -I ".gpg-id" "$PREFIX/$path" 3>&- | tail -n +2 | sed -E 's/\.gpg(\x1B\[[0-9]+m)?( ->|$)/\1\2/g' # remove .gpg at end of line, but keep colors
	elif [[ -z $path ]]; then
...
cmd_find() {
	...
	tree -N -C -l --noreport -a -I ".git/" -I ".gitignore" -I ".gpg-id" -P "${terms%|*}" --prune --matchdirs --ignore-case "$PREFIX" 3>&- | tail -n +2 | sed -E 's/\.gpg(\x1B\[[0-9]+m)?( ->|$)/\1\2/g'
}
